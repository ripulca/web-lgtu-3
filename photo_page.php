        ...
        <main class="main">
            <div class='main_photo_container'>
                <?php
                    ...
                    $sql = "SELECT photo_name, photo_path FROM photos WHERE photo_id=".$photo_id;
                    $proc=$db_connection->prepare($sql);
                    // $proc->bindParam(':photo_id', $photo_id);
                    $proc->execute();
                    $db_connection->query($sql);
                    $photo=$proc->fetch(PDO::FETCH_ASSOC);
                    echo '<img src="'.$photo['photo_path'].'" width="30%" height="auto"/>';
                ?>
            </div>
